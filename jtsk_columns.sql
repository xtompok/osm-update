...
SELECT AddGeometryColumn ('public','nodes','jtsk_geometry',5514,'POINT',2);
SELECT AddGeometryColumn ('public','ways','jtsk_geometry',5514,'LINESTRING',2);
...
UPDATE nodes SET jtsk_geometry = ST_Transform(geom,5514);
UPDATE ways SET jtsk_geometry = ST_Transform(linestring,5514);
...
CREATE INDEX nodes_jtsk ON nodes USING GIST(jtsk_geometry);
CREATE INDEX ways_jtsk ON ways USING GIST(jtsk_geometry);
